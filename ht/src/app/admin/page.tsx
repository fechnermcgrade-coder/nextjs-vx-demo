...
export default function AdminPage() {
  ...
  const recentPosts = reviewablePosts.slice(0, 6);
  ...
  if (token === null) return <Panel><p className="py-16 text-center text-slate-500">正在恢复登录状态...</p></Panel>;
  if (!token) return <section className="mx-auto max-w-md rounded-[28px] border border-[#d9e1df] bg-[#fbfaf6] p-7 shadow-xl shadow-[#d9e1df]/60"><p className="text-sm font-black uppercase text-[#2f5d62]">Vitex Admin</p><h1 className="mt-3 text-3xl font-black text-slate-950">管理后台登录</h1><p className="mt-3 leading-7 text-slate-600">请输入管理员邮箱和密码。普通用户账号无法进入工作台。</p><form onSubmit={login} className="mt-7 space-y-4"><label className="block text-sm font-bold text-slate-700">管理员邮箱<input className="mt-2 w-full rounded-2xl border border-[#d9e1df] bg-white px-4 py-3 outline-none focus:border-[#2f5d62]" value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="email" /></label><label className="block text-sm font-bold text-slate-700">密码<input className="mt-2 w-full rounded-2xl border border-[#d9e1df] bg-white px-4 py-3 outline-none focus:border-[#2f5d62]" value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete="current-password" /></label><button className="w-full rounded-2xl bg-[#2f5d62] px-5 py-3 font-black text-white disabled:opacity-50" disabled={busy === "login"} type="submit">{busy === "login" ? "登录中..." : "进入工作台"}</button></form>{message && <p className="mt-4 text-sm text-slate-600">{message}</p>}</section>;
...
function StatusSummary({ published, pending, returned }: { published: number; pending: number; returned: number }) {
  const total = Math.max(1, published + pending + returned);
  const publishedEnd = (published / total) * 100;
  const pendingEnd = publishedEnd + (pending / total) * 100;
  const items = [
    { label: "已发布", value: published, color: "#2f5d62" },
    { label: "待审核", value: pending, color: "#c9a66b" },
    { label: "退回草稿", value: returned, color: "#b42318" }
  ];
  const background = published + pending + returned
    ? `conic-gradient(#2f5d62 0 ${publishedEnd}%, #c9a66b ${publishedEnd}% ${pendingEnd}%, #b42318 ${pendingEnd}% 100%)`
    : "#eef4f3";
  return <div className="grid gap-5 md:grid-cols-[180px_1fr] md:items-center"><div className="mx-auto grid h-40 w-40 place-items-center rounded-full" style={{ background }}><div className="grid h-24 w-24 place-items-center rounded-full bg-white text-center"><span className="text-2xl font-black text-slate-950">{published + pending + returned}</span><span className="-mt-4 text-xs font-bold text-slate-500">篇</span></div></div><div className="space-y-3">{items.map((item) => <div key={item.label} className="flex justify-between rounded-2xl bg-slate-50 px-4 py-3"><span className="flex items-center gap-2 font-bold text-slate-700"><span className="h-3 w-3 rounded-full" style={{ backgroundColor: item.color }} />{item.label}</span><span className="font-black">{item.value}</span></div>)}</div></div>;
}
